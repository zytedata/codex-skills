...
Expect minutes per call. It prints `Crawled N pages` to stderr every 15 seconds; half-filled page directories and log entries mid-run are normal. Set `yield_time_ms` up to Codex's effective 30000 ms cap. If the command is still running and returns a `session_id`, poll it with `write_stdin` using empty stdin until the command exits. NEVER interrupt it, and NEVER start a second call while one is running.
