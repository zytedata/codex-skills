...
the job is done, which can take minutes. Set `yield_time_ms` up to Codex's effective 30000 ms cap. If the command is still running and returns a `session_id`, poll it with `write_stdin` using empty stdin until the command exits. Progress goes to
